...
def NN(WriteFile=True):
    ...
    model = Sequential()
    model.add(SimpleRNN(2048, return_sequences=False, input_shape=(x_ref.shape[1],x_ref.shape[2])))
    model.add(Dense(128,activation='sigmoid'))
    model.add(Dense(128,activation='softmax'))
    model.compile(loss='categorical_crossentropy', optimizer='adam',metrics=['accuracy'])
    model.summary()
    history = model.fit(x_ref,y_ref,validation_data=(x_comperison,y_comperison), batch_size=32, epochs=10)
